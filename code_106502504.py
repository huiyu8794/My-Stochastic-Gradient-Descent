...
def mf_sgd(R, K=64, alpha=1e-4, beta=1e-2, iterations=50):
    ...
    d = np.zeros((num_users, num_items))
    ...
    for iters in range(iterations):
        ...
        for i, j, r in samples:
            ...
            you need to (1)update "b_u"(vector of rating bias for users) 
            and "b_i"(vector of rating bias for items)
            and (2)update user and item latent feature matrices "P", "Q"
            """
            d[i,j] = R[i,j] - pred[i,j]
    
            b_u[i] = b_u[i] - alpha*(-d[i,j] + beta*b_u[i])
            b_i[j] = b_i[j] - alpha*(-d[i,j] + beta*b_i[j])
            P[i,:] = P[i,:] - alpha*(-d[i,j]*Q[j,:] + beta*P[i,:])
            Q[j,:] = Q[j,:] - alpha*(-d[i,j]*P[i,:] + beta*Q[j,:])
